...
class Controller:
    ...
    def run(self):
        ...
        for i in range(1, len(self.images)):
            ...
            tfl_curr = [merge_points[i] for i in range(len(tfl_bool_prev)) if tfl_bool_prev[i] == 1]
            self.sfm.set_curr_prev(self.images[i], self.images[i - 1], tfl_curr, tfl_prev)
            curr_container, rot_pts, foe = self.sfm.run()
            self.view.set_params(curr_container, rot_pts, foe)
